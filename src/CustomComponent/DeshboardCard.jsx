          ...
          {data?.map((item, index) => (
            <div key={index} className="px-4 py-3 flex gap-10 items-center">
              <div className="flex-1">
                <p className="hover:font-semibold transition-all duration-300 cursor-pointer">{item.title}</p>
              </div>
          ...
          {data.map((assignment, index) => (
            <div key={index} className="p-4">
              <div className="flex justify-between">
                <h3 className="hover:font-semibold transition-all duration-300 cursor-pointer">{assignment.course}</h3>
  ...
  return (
    <Link key={course.id} to={`/studentPortal/allCourseDetails/${course.id}`}>
      <Card className="w-full overflow-hidden cursor-pointer gap-0 py-0">
        ...
        <div className="p-4">
          <div className="uppercase text-indigo-600 bg-indigo-100 text-xs font-medium mb-2 w-fit px-2">{course.category || "Developments"}</div>
          <h3 className="text-xl font-bold text-gray-900 mb-4">{course.course}</h3>
          <div className="text-xl font-bold text-green-500 mb-3">${course.Prise || "24.00"}</div>
